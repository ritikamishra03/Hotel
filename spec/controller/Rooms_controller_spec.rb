    ...
    describe "POST #create" do
        context "with valid params" do
            it "creates a new Room" do
                expect {
                    post :create, params: { room: { room_number: 101, room_type_id: room_type.id, status: "available" } }
    ...
    describe "PATCH #update" do
        context "with valid params" do
            it "updates the room and redirects to the rooms path" do
                patch :update, params: { id: room.id, room: { room_number: 102, room_type_id: room_type.id, status: "available" } }
                room.reload
                expect(room.room_number)==(102)
                expect(response).to redirect_to(rooms_path)
        ...
        context "with invalid params" do
            it "renders the edit template" do
                patch :update, params: { id: room.id, room: { room_number: nil, room_type_id: room_type.id, status: "available" } }
